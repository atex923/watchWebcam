...
class FrameCapturer:
    ...
    def resolve_youtube_stream_url(self, youtube_url: str) -> str:
        ...
        if self.youtube_cookies_from_browser:
            ydl_opts["cookiesfrombrowser"] = self.parse_cookies_from_browser(
                self.youtube_cookies_from_browser
            )
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            try:
                ...
                if "Sign in to confirm" in message or "not a bot" in message:
                    message += (
                        "；YouTube 要求驗證，請使用 --youtube-cookies-from-browser "
                        "chrome/safari/firefox，或 --youtube-cookies cookies.txt"
                    )
                raise RuntimeError(f"YouTube 串流解析失敗：{message}") from exc
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--youtube-cookies", type=Path, default=None, help="yt-dlp 使用的 YouTube cookies.txt")
    parser.add_argument(
        "--youtube-cookies-from-browser",
        default="",
        help="從本機瀏覽器讀取 YouTube cookies，例如 chrome、safari、firefox",
    )
...
def main() -> int:
    args = parse_args()
    FrameCapturer.youtube_cookies = args.youtube_cookies
    FrameCapturer.youtube_cookies_from_browser = args.youtube_cookies_from_browser
