...
class FrameCapturer:
    ...
    def fetch_image(self) -> Image.Image:
        ...
        if kind in {"webpage_screenshot", "webpage", "browser_page", "page_capture"}:
            return self.fetch_webpage_screenshot()
    ...
    def fetch_webpage_screenshot(self) -> Image.Image:
        if sync_playwright is None:
            raise RuntimeError("缺少 Playwright，請先執行：python3 -m pip install playwright && python3 -m playwright install chromium")
        url = self.camera.source_page or self.camera.url
        if not url:
            raise RuntimeError("沒有可開啟的來源網頁 URL")
        with sync_playwright() as playwright:
            browser = playwright.chromium.launch(headless=True)
            try:
                page = browser.new_page(viewport={"width": 1280, "height": 720}, device_scale_factor=1)
                page.goto(url, wait_until="domcontentloaded", timeout=self.request_timeout * 1000)
                self.wait_for_page_video(page)
                target = self.pick_screenshot_target(page)
                if target:
    ...
    def wait_for_page_video(self, page: object) -> None:
        deadline = time.time() + self.request_timeout
        selectors = ("iframe[src*='youtube']", ".video-container", ".product-detail-content")
    ...
    @staticmethod
    def pick_screenshot_target(page: object) -> object:
        for selector in ("iframe[src*='youtube']", ".video-container", ".product-detail-content"):
            try:
                element = page.query_selector(selector)
            except Exception:
                element = None
